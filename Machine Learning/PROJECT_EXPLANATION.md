...
6. **Giải thích mô hình (SHAP):** Sử dụng SHAP (SHapley Additive exPlanations) để giải thích quyết định của mô hình thông qua biểu đồ Waterfall.
7. **Cơ sở dữ liệu:** PostgreSQL lưu lịch sử giao dịch với hỗ trợ JSONB và định danh mô hình.
...
| `/explain-raw/`       | POST   | Tạo biểu đồ SHAP từ dữ liệu thô và model_id                  |
| `/analyze-shap/`      | POST   | AI phân tích dữ liệu toán học SHAP để đưa ra giải thích tự nhiên |
...
Webapp hỗ trợ **2 nhà cung cấp AI** độc lập. Người dùng có thể chọn các mô hình AI khác nhau cho tính năng Phân tích Lô (Batch Analysis) và tính năng Trợ lý ảo (Chatbot) mà không bị xung đột cấu hình:
...
### 4.6. Chế độ Hoạt động (Demo vs Nghiệp vụ)

Ứng dụng được chia làm 2 luồng công việc chính để phục vụ các mục đích khác nhau:
1. **Chế độ Demo (Giao dịch đơn lẻ):** Nhập tay từng giao dịch để kiểm tra mô hình. Dữ liệu được lưu với `batch_id = NULL`.
2. **Chế độ Nghiệp vụ (Xử lý theo Lô):** Cho phép upload file Excel/CSV chứa hàng nghìn giao dịch. Hệ thống sẽ dự đoán hàng loạt, gán một `batch_id` duy nhất (VD: `batch_20260514_140000`) và đánh số thứ tự lô (Lô #1, Lô #2) để quản lý.

3. **Giải thích bằng AI & SHAP (Explainable AI):**
   - Người dùng bấm nút "Giải thích" cạnh từng giao dịch.
   - Hệ thống gọi API `/explain-raw/` để vẽ biểu đồ đóng góp của các tính năng.
   - Gọi API `/analyze-shap/` để AI giải thích ý nghĩa các con số SHAP bằng ngôn ngữ dễ hiểu.
   - Giao diện hiển thị Biểu đồ (Trái) và AI phân tích (Phải) đồng nhất.

**Cách ly dữ liệu:** Nhờ thiết kế `batch_id`, dữ liệu của phần Demo và phần Nghiệp vụ hoàn toàn độc lập, không bị trộn lẫn khi truy vấn lịch sử hoặc hiển thị trên giao diện.
